...
## implement Fernandez and Steel skew normal distribution for the base distribution 
library(R2jags)
set.seed(1508)
...
model {
  ...
  for (k in 1:N) {
   ...
   # Priors
      ...
      omega ~ dunif(0,10)
      shape ~ dnorm(0.1,0.04)
      ## shape ~ dgamma(0.5, 0.318) a prior they propose

   # base_mu <- xi + omega*((shape)/sqrt(1+(shape^2)))
   # base_tau2 <- omega_sqr*((1- (2/pi))*((shape)^2 /(1+shape^2)))
...
}", file = "DPp_26_U_U_sn_base.txt")
modfile = 'DPp_26_U_U_sn_base.txt'


run.modelDPp26_U_U_sn_base = jags(
...
DPpresults_U_U_26_skew_base <- as.data.frame(run.modelDPp26_U_U_sn_base$BUGSoutput$summary) 

DPp_results_U_U_26_skew_base <- as.data.frame(run.modelDPp26_U_U_sn_base$BUGSoutput$summary) 
...
library(coda)
mcmc_obj <- as.mcmc(run.modelDPp26_U_U_sn_base)
...
traceplot(mcmc_obj[, c("xi", "omega", "alpha", "delta_new", 
                       "poptrue", "shape",
                       "var.true")])
...
######## Pr(mu<0) #######
Pr_mu_DPp_U_U_26_skew_base  = mean(run.modelDPp26_U_U_sn_base$BUGSoutput$sims.matrix[  ,"poptrue"] < 0 )
######## Pr(mu_new<0) #######
Pr_delta_new_DPp_U_U_26_skew_base = mean(run.modelDPp26_U_U_sn_base$BUGSoutput$sims.matrix[  ,"delta_new"] < 0 )


### extract co-clustering probabilities
sims_equals <- run.modelDPp26_U_U_sn_base$BUGSoutput$sims.matrix
...
resDPp_U_U_26_skew_base <- cbind.data.frame(
...
# 
# write.csv(resDPp_U_U_26_skew_base, "C:\\Users\\Lela Panag\\Desktop\\2nd PhD article\\data\\pre_term_data58\\DPp_U_U_26_sn_base_pred_58\\corrections\\2res_DPp_26_U_U_skew_base_pred.csv",row.names=FALSE )  
# write.csv(listaDPp_U_U_26_skew_base, "C:\\Users\\Lela Panag\\Desktop\\2nd PhD article\\data\\pre_term_data58\\DPp_U_U_26_sn_base_pred_58\\corrections\\2rel_eff_DPp_26_U_U_skew_base_pred.csv",row.names=FALSE )  
# write.csv(prob_DPp_U_U_26_skew_base, "C:\\Users\\Lela Panag\\Desktop\\2nd PhD article\\data\\pre_term_data58\\DPp_U_U_26_sn_base_pred_58\\corrections\\2max_prob_cluster_DPp_26_U_U_skew_base_pred.csv",row.names=FALSE )  
# write.csv(DPpresults_U_U_26_skew_base, "C:\\Users\\Lela Panag\\Desktop\\2nd PhD article\\data\\pre_term_data58\\DPp_U_U_26_sn_base_pred_58\\corrections\\2all_res_DPp_26_U_U_skew_base_pred.csv",row.names=FALSE )  
