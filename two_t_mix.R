...
writeLines("
  model {
    ...
    ###### Assigning a Dirichlet prior for p reasures that sum[p]=1 ###
    ...
    for (k in 1:N) {
      ...
      tau1[k] ~ dgamma(3,b) ## variation of tau across components

      # Variance of t component
      tau_k_sqr[k] <- (df[k] / (df[k] - 2)) * (1 / tau1[k])
  ...
  pred ~ dcat(p[1:N])
  delta_new ~ dt(mu[pred],tau1[pred], df[pred])
...
res_fin_mix_t_dw = as.data.frame(run.model_fin_mix_t_dir_weigths_tau_comp$BUGSoutput$summary)
res_fin_mix_t_dw1 = round(res_fin_mix_t_dw, 2)
View(res_fin_mix_t_dw1)

#write.csv(res_fin_mix_t_dw1, "C:\\Users\\Lela Panag\\Desktop\\2nd PhD article\\data\\pre_term_data58\\two_t_mix_58\\dir_weights\\gamma_comp_var\\tauinv2_gamma\\2res_all.csv",row.names=TRUE )  

##### to check the convergence ###########
library(coda)
mcmc_obj <- as.mcmc(run.model_fin_mix_t_dir_weigths_tau_comp)
...
traceplot(mcmc_obj[, grep("^mu\\[", varnames(mcmc_obj[[1]]))][,1:2]) # first 6 clusters
traceplot(mcmc_obj[, grep("^tau_k_sqr\\[", varnames(mcmc_obj[[1]]))][,1:2]) # first 6 clusters
traceplot(mcmc_obj[, grep("^tau1\\[", varnames(mcmc_obj[[1]]))][,1:2]) # first 6 clusters
...
p.bias.1 <- run.model_fin_mix_t_dir_weigths_tau_comp$BUGSoutput$sims.list$p[,2]
mu     <- run.model_fin_mix_t_dir_weigths_tau_comp$BUGSoutput$sims.list$mu
tau   <- run.model_fin_mix_t_dir_weigths_tau_comp$BUGSoutput$sims.list$tau_comp
...
######## cluster assignments ######
fdd <- grepl("z", row.names(res_fin_mix_t_dw))
mdd <- res_fin_mix_t_dw[fdd,]
...
# 
# write.csv(cluster_mix_t_1, "C:\\Users\\Lela Panag\\Desktop\\2nd PhD article\\data\\pre_term_data58\\two_t_mix_58\\dir_weights\\gamma_comp_var\\tauinv2_gamma\\2cluster1_assign_fin_mix_t_dw.csv",row.names=FALSE )  
# write.csv(cluster_mix_t_2, "C:\\Users\\Lela Panag\\Desktop\\2nd PhD article\\data\\pre_term_data58\\two_t_mix_58\\dir_weights\\gamma_comp_var\\tauinv2_gamma\\2cluster2_assign_fin_mix_t_dw.csv",row.names=FALSE )  

###### study-specific effects
fdd <- grepl("delta12", row.names(res_fin_mix_t_dw))
mdd <- res_fin_mix_t_dw[fdd,]
...
# write.csv(mean_cl, "C:\\Users\\Lela Panag\\Desktop\\2nd PhD article\\data\\pre_term_data58\\two_t_mix_58\\dir_weights\\gamma_comp_var\\tauinv2_gamma\\2mean_cl.csv",row.names=FALSE )  

########## CLUSTER ASSIGNEMENT ##########
#### BASED ON  THE max(SC[i,j]) ####
res_fin_mix_t_dw$ind <- row.names(res_fin_mix_t_dw)

############# extraction of the parameters of interest ###########
f55sc_t_mix <- grepl("SC", row.names(res_fin_mix_t_dw))
m55sc_t_mix <- res_fin_mix_t_dw[f55sc_t_mix,]
...
# 
# write.csv(max_values, "C:\\Users\\Lela Panag\\Desktop\\2nd PhD article\\data\\pre_term_data58\\two_t_mix_58\\dir_weights\\gamma_comp_var\\tauinv2_gamma\\2cluster_prob_pred.csv",row.names=FALSE )
# write.csv(max_indices, "C:\\Users\\Lela Panag\\Desktop\\2nd PhD article\\data\\pre_term_data58\\two_t_mix_58\\dir_weights\\gamma_comp_var\\tauinv2_gamma\\2cluster_assignment.csv",row.names=FALSE )
...
f33_fin_mix_t_dir_weigths <- grepl("mu", row.names(res_fin_mix_t_dw))
m33_fin_mix_t_dir_weigths <- res_fin_mix_t_dw[f33_fin_mix_t_dir_weigths,]
...
#write.csv(cl_mu_fin_mix_t_dw , "C:\\Users\\Lela Panag\\Desktop\\2nd PhD article\\data\\pre_term_data58\\two_t_mix_58\\dir_weights\\gamma_comp_var\\tauinv2_gamma\\component_means.csv",row.names=FALSE )  

#### Variance of each component
f44_fin_mix_t_dir_weigths <- grepl("tau_k_sqr", row.names(res_fin_mix_t_dw))
m44_fin_mix_t_dir_weigths <- res_fin_mix_t_dw[f44_fin_mix_t_dir_weigths,]
...
f11_fin_mix_t_dir_weigths <- grepl("total_mean", row.names(res_fin_mix_t_dw))
m11_fin_mix_t_dir_weigths <- res_fin_mix_t_dw[f11_fin_mix_t_dir_weigths,]
...
f17_fin_mix_t_dir_weigths <- grepl("delta_new", row.names(res_fin_mix_t_dw))
m17_fin_mix_t_dir_weigths <- res_fin_mix_t_dw[f17_fin_mix_t_dir_weigths,]
...
f22_fin_mix_t_dir_weigths <- grepl("total_var", row.names(res_fin_mix_t_dw))
m22_fin_mix_t_dir_weigths <- res_fin_mix_t_dw[f22_fin_mix_t_dir_weigths,]
...
fp_fin_mix_t_dir_weigths <- grepl("p", row.names(res_fin_mix_t_dw))
mp_fin_mix_t_dir_weigths <- res_fin_mix_t_dw[fp_fin_mix_t_dir_weigths,]
